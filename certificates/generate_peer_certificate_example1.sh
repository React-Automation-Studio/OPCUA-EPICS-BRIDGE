...
openssl req -x509 -newkey rsa:2048 -keyout peer-private-key-example-1.pem -out peer-certificate-example-1.pem -days 355 -nodes -addext "subjectAltName = URI:urn:example.org:FreeOpcUa:python-opcua"
openssl x509 -outform der -in peer-certificate-example-1.pem -out peer-certificate-example-1.der
